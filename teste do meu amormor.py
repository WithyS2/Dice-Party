...
class TelaJogo(TelaBase):
    ...
    def processar_eventos(self, eventos):
        global contagem_duck, contagem_cat, coins_duck_1, coins_cat_1, star_duck_1, star_cat_1
        for event in eventos:
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                ...
                casa_amormor = contagem_duck - dado
                if casa_amormor <= 10 < contagem_duck or casa_amormor == 10 and contagem_duck == 1:
                    exibir_mensagem(screen, f"Quer comprar uma estrela?", 2000, 1280, 500)
                
                    if event.type == pygame.KEYDOWN and coins_duck_1 >= 20:
                        exibir_mensagem(screen,f'Você comprou uma estrela! ★', 2000, 1280, 820)
                        star_duck_1 += 1
                    else:
                        exibir_mensagem(screen,f'Você não pode comprar uma estrela!', 2000, 1280, 820)
    ...
    def desenhar(self, screen):
        screen.fill((0,0,0))
        star_duck_formatado = font_points.render(f'{star_duck_1}', True, (0,0,0))
        star_cat_formatado = font_points.render(f'{star_cat_1}', True, (0,0,0))
